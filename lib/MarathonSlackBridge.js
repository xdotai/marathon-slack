...
function MarathonSlackBridge (options) {
    ...
    self.eventTypes.forEach(function (eventType) {
        ...
        marathonOptions.handlers[eventType] = function (type, data) {
            ...
            if ( recentEvents[data.appId] ) {
                dateDiff = currentDate - recentEvents[data.appId] 
                if(dateDiff > defaultWaitTimeMs) {
                    emit()
                }
                else return
            }
            else {
                recentEvents[data.appId] = currentDate
                emit
            }
        }
    });
